...
k_means_nodr_helix = kmeans(helix_data,centers = 5)
...
# Dunn index
dunn_k_means_helix_nodr <- dunn( distance = NULL, clusts_nodr_helix, Data = helix_data)
...
cut_agnes_helix_nodr <- cutree(agnes_helix_nodr, k = 5)
...
dunn_agnes_helix_nodr <- dunn( dist(helix_data), cut_agnes_helix_nodr)
...
#kmeans
set.seed(123)
k_means_pca_helix = kmeans(helix_pca,centers = 5)
...
# Dunn index
dunn_k_means_helix_pca <- dunn( distance = NULL, clusters_pca_helix, Data = helix_pca)
...
cut_agnes_helix_pca <- cutree(agnes_helix_pca, k = 5)
...
dunn_agnes_helix_pca <- dunn( dist(helix_pca), cut_agnes_helix_pca)
...
#k-means with t-sne
set.seed(123)
k_means_tsne_helix = kmeans(tsne_helix,centers = 5)
...
# Dunn index
dunn_k_means_helix_tsne <- dunn( distance = NULL, clusters_tsne_helix, Data = tsne_helix)
...
cut_agnes_helix_tsne <- cutree(agnes_helix_tsne, k = 5)
...
dunn_agnes_helix_tsne <- dunn( dist(tsne_helix), cut_agnes_helix_tsne)
...
library(uwot)
set.seed(123)
umap_helix <- umap(helix_data, n_neighbors = 50, min_dist = 0.1, n_components = 2, 
                 init = "laplacian", learning_rate = 5.0)
#Kmeans Umap
set.seed(123)
kmeans_helix_umap <- kmeans(umap_helix, centers = 5)
...
# Dunn index
dunn_k_means_helix_umap <- dunn( distance = NULL, kmeans_cluster_helix_umap, Data = umap_helix)
...
cut_agnes_umap_helix <- cutree(agnes_cluster_umap_helix, k = 5)
...
#scatterplot3d(helix_data, color=rainbow(4)[cut_agnes_umap_helix], main = "agnes after UMAP helix")
...
dunn_agnes_helix_umap <- dunn( distance = NULL, cut_agnes_umap_helix, Data = umap_helix)
