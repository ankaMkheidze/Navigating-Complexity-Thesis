...
set.seed(123)
k_means_nodr_hd = kmeans(hd_data,centers = 3)
...
# Dunn index
dunn_k_means_hd_nodr <- dunn( distance = NULL, clusts_nodr_hd, Data = hd_data)
...
cut_agnes_hd_nodr <- cutree(agnes_hd_nodr, k = 3)
...
dunn_agnes_hd_nodr <- dunn( dist(hd_data), cut_agnes_hd_nodr)
...
#kmeans
set.seed(123)
k_means_pca_hd = kmeans(hd_pca,centers = 3)
...
# Dunn index
dunn_k_means_hd_pca <- dunn( distance = NULL, clusters_pca_hd, Data = hd_pca)
...
cut_agnes_hd_pca <- cutree(agnes_hd_pca, k = 3)
...
dunn_agnes_hd_pca <- dunn( dist(hd_pca), cut_agnes_hd_pca)
...
#k-means with t-sne
set.seed(123)
k_means_tsne_hd = kmeans(tsne_hd,centers = 3)
...
# Dunn index
dunn_k_means_hd_tsne <- dunn( distance = NULL, clusters_tsne_hd, Data = tsne_hd)
...
cut_agnes_hd_tsne <- cutree(agnes_hd_tsne, k = 3)
...
dunn_agnes_hd_tsne <- dunn( dist(tsne_hd), cut_agnes_hd_tsne)
...
library(uwot)
set.seed(123)
umap_hd <- umap(hd_data, n_neighbors = 25, min_dist = 0.001, n_components = 2, 
                 init = "laplacian", learning_rate = 5.0)
#Kmeans Umap
set.seed(123)
kmeans_hd_umap <- kmeans(umap_hd, centers = 3)
...
# Dunn index
dunn_k_means_hd_umap <- dunn( distance = NULL, kmeans_cluster_hd_umap, Data = umap_hd)
...
cut_agnes_umap_hd <- cutree(agnes_cluster_umap_hd, k = 3)
...
#scatterplot3d(hd_data, color=rainbow(4)[cut_agnes_umap_hd], main = "agnes after UMAP hd")
...
dunn_agnes_hd_umap <- dunn( distance = NULL, cut_agnes_umap_hd, Data = umap_hd)
